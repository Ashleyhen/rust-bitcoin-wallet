...
impl AddressSchema for P2TR{
    ...
    fn prv_tx_input(&self,previous_tx:Vec<Transaction>,current_tx:Transaction) ->(Vec<Input>, Transaction) {
        ...
        let tx_out_list:Vec<TxOut>=previous_tx.iter().enumerate().flat_map(|(i, previous_tx)|{
        ...
        tap_key_origin.insert(signer_pub_k.to_x_only_pub(),
      ...
      return tx_output;
        ...
        let uxto=&tx_out_list.clone()[0];
        new_input.witness_utxo=Some(uxto.clone());
        let sig_hash=SighashCache::new(&mut current_tx.clone())
                  .taproot_key_spend_signature_hash( 0, &Prevouts::All(&tx_out_list), SchnorrSighashType::AllPlusAnyoneCanPay).unwrap();
        ...
        return (vec![new_input],current_tx);
    }
