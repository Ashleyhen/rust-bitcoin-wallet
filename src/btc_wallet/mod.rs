...
use bitcoin::{
    util::{
        bip32::{ExtendedPubKey, KeySource},
    },
};
...
use self::{
    unlock::SignTx,
    wallet_traits::{AddressSchema, ApiCall}, wallet_methods::{ClientWithSchema, Broadcast_op}, input_data::electrum_rpc::ElectrumRpc, lock::pub_key_lock, 
};
// pub mod input_data;
pub mod input_data;
...
impl<'a, S: AddressSchema> ClientWithSchema<'a, S, ElectrumRpc>{
    ...
    self.schema.to_wallet().finalize(psbt, &|tx| match broad_cast_op {
