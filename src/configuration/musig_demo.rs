...
use bitcoin::{secp256k1::Secp256k1, Address, util::bip32::{ExtendedPrivKey, ExtendedPubKey}};
use crate::bitcoin_wallet::{address_formats::{derive_derivation_path, generate_key_pair}, constants::{SEED, NETWORK}, spending_path::{musig::{MuSig_Script, self}, tap_script_spending_ex::TapScriptSendEx, p2tr_key_path::P2tr}, script_services::psbt_factory::{default_output, get_output, create_partially_signed_tx, merge_psbt}, input_data::regtest_rpc::RegtestRpc};


pub fn musig_demo(){
let secp = Secp256k1::new();
    let get_xonly_fn=|i,j|derive_derivation_path(generate_key_pair(Some(SEED.to_string())),32)
    (j,i).to_keypair(&secp);
    ...
    ;
    let ext_pub=ExtendedPubKey::from_priv(&secp, &ExtendedPrivKey::new_master(NETWORK, &key.secret_bytes()).unwrap());
