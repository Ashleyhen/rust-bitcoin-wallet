...
pub fn map_seeds_to_scripts<'a>(
    ...
    return Box::new(move |recieve, index| {
        return map_to_addr(secp,ExtendedPubKey::from_priv(&secp,&extended_priv_key.derive_priv(&secp, &get_derivation_p(purpose,recieve,index)).unwrap())
        );
    });
}
...
pub fn derive_derivation_path(
    ...
    return Box::new(move |recieve, index| {
        return extended_priv_key.derive_priv(&secp, &get_derivation_p(purpose,recieve,index)).unwrap();
    });
}
