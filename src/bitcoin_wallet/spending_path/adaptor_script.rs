use bitcoin::{
    ...
    secp256k1::{
        ...
        All, Scalar, Secp256k1, SecretKey,
    },
    util::taproot::TaprootBuilder,
    Address, KeyPair, Script, Transaction, TxOut, Witness, XOnlyPublicKey,
};

use crate::bitcoin_wallet::{
    constants::{Seed, NETWORK, TIP},
    script_services::{
        input_service::{insert_control_block, sign_2_of_2, sign_tapleaf},
        output_service::{
...
impl<'a> AdaptorScript<'a> {
    ...
    pub fn adaptor_sig(
        ...
        let tap_spending_info = tap_builder
            .finalize(self.secp, untweaked_internal.clone())
            .unwrap();
    ...
    pub fn finialize_script(psbt: PartiallySignedTransaction) -> Transaction {
        let mut witness = Witness::new();
        psbt.inputs
            .iter()
            .flat_map(|f| f.tap_script_sigs.iter().map(|sig| sig.1))
            .for_each(|shnor| {
