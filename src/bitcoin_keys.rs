use std::{str::FromStr, hash::{Hash, Hasher}, collections::hash_map::DefaultHasher};
use bdk::bitcoin::util::bip32::Fingerprint;
use bitcoin::{secp256k1::{ Secp256k1, KeyPair,rand,PublicKey, SecretKey },util::{address, bip32::{ExtendedPrivKey, ExtendedPubKey, DerivationPath}}, Network,  XOnlyPublicKey, hashes::hex::FromHex};
...
impl Bitcoin_keys{
	...
	pub fn new(secret_key:Option<String> ) ->  Bitcoin_keys {
		...
		println!("stset{:#?}",p2wpkh_address.hash(&mut DefaultHasher::new()));
		return Bitcoin_keys { 
