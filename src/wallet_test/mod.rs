...
use bitcoin::{
    ...
    util::{
        ...
        sighash::ScriptPath,
        taproot::{
            ControlBlock, LeafVersion, ScriptLeaf, TapLeafHash, TapLeafTag, TapSighashHash,
            TapSighashTag,
        },
    },
    Address, PrivateKey, SchnorrSighashType, Script, SigHashType, Transaction, TxOut,
    XOnlyPublicKey,
};

use self::wallet_test_vector_traits::{Expected, UtxosSpent};
...
impl UtxosSpent {
    pub fn get_tx_out(&self) -> TxOut {
        return TxOut {
            value: self.amount_sats,
            script_pubkey: Script::from_hex(&self.script_pub_key).unwrap(),
        };
    }
}
...
impl ScriptTree {
    pub fn get_script_leaf(&self) -> TapLeafHash {
        return TapLeafHash::from_script(
            &Script::from_hex(&self.script).unwrap(),
...
impl Expected {
    pub fn get_bip_350_address(&self) -> Option<Address> {
        return self
            .bip350address
            .as_ref()
            .map(|f| Address::from_str(&f).unwrap());
    }
    ...
    pub fn get_witness(&self) -> Vec<Script> {
        return self
            .witness
            .iter()
            .map(|w| Script::from_hex(w).unwrap())
            .collect::<Vec<Script>>();
    }
    ...
    pub fn get_script_patch_control_block(&self) -> Vec<ControlBlock> {
        return self
            ...
            .iter()
            .map(|w| ControlBlock::from_slice(&Vec::from_hex(w).unwrap().clone()).unwrap())
...
impl Given {
    ...
    pub fn get_hash(&self) -> Option<SchnorrSighashType> {
        return self
            .hash_type
            .map(|f| SchnorrSighashType::from_u8(f).unwrap());
    }
    ...
    pub fn get_unsigned_tx(&self) -> Option<Transaction> {
        return self
            .raw_unsigned_tx
            .as_ref()
            .map(|w| Transaction::deserialize(&Vec::from_hex(w).unwrap()).unwrap());
    }
    ...
    pub fn get_internal_pubkey(&self) -> Option<XOnlyPublicKey> {
        return self
            .internal_pubkey
            .as_ref()
            .map(|f| XOnlyPublicKey::from_str(f).unwrap());
    }
